...
def find_tests_using_node(name, env):
   TEST_GROUP = []
   l = glob.glob(os.path.join(env.Dir('#').abspath, 'testsuite', env['TEST_PATTERN']))
...
def find_test_group(group, env):
   ...
   # Special 'all' group is expanded to the whole testsuite, including the 'ignored' tests
      ...
      if os.path.exists(os.path.join(env.Dir('#').abspath, 'testsuite', 'groups.local')):
         (TEST_GROUP, found, merge) = find_test_group_in_file(group, os.path.join(env.Dir('#').abspath, 'testsuite', 'groups.local'))
         
      # If not found, then search the global test groups mapping file
      if merge or not found:
         (TEST_GROUP2, found, merge) = find_test_group_in_file(group, os.path.join(env.Dir('#').abspath, 'testsuite', 'groups'))
...
def get_next_test_name():
   l = glob.glob(os.path.join(env.Dir('#').abspath, "testsuite", 'test_[0-9][0-9][0-9][0-9]'))
   l.sort()
...
def get_test_list(groups, env, PATTERNS, TAGS):
   ...
   if not groups:
      ...
      for name in glob.glob(os.path.join(env.Dir('#').abspath, 'testsuite', pattern)):
      ...
      for tag in tags:
         if tag.startswith('test_'):
            PATTERNS.append(tag)
            for name in glob.glob(os.path.join(env.Dir('#').abspath, 'testsuite', tag)):
               tests.append(os.path.join(os.path.basename(name)))
         else:
            TAGS.append(tag)
