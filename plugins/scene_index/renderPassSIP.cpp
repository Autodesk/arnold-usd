...
TF_DEFINE_PRIVATE_TOKENS(
    ...
    (matte)
    (prune)
    ((sceneIndexPluginName, "HdArnoldRenderPassSceneIndexPlugin"))
);
// clang-format on

/* static */
HdArnoldRenderPassSceneIndexRefPtr HdArnoldRenderPassSceneIndex::New(const HdSceneIndexBaseRefPtr &inputSceneIndex)
{
    return TfCreateRefPtr(new HdArnoldRenderPassSceneIndex(inputSceneIndex));
}
...
{
    return _IsGeometryType(primType) || HdPrimTypeIsLight(primType) || primType == HdPrimTypeTokens->lightFilter;
}
...
bool HdArnoldRenderPassSceneIndex::_RenderPassState::DoesOverrideMatte(
    SdfPath const &primPath, HdSceneIndexPrim const &prim) const
{
    return matteEval && _IsGeometryType(prim.primType) && matteEval->Match(primPath);
}

bool HdArnoldRenderPassSceneIndex::_RenderPassState::DoesOverrideVis(
    SdfPath const &primPath, HdSceneIndexPrim const &prim) const
{
    return renderVisEval && _ShouldApplyPassVisibility(prim.primType) && !renderVisEval->Match(primPath) &&
           _IsVisible(prim.dataSource);
}

bool HdArnoldRenderPassSceneIndex::_RenderPassState::DoesOverrideCameraVis(
    SdfPath const &primPath, HdSceneIndexPrim const &prim) const
{
    return cameraVisEval && _ShouldApplyPassVisibility(prim.primType) && !cameraVisEval->Match(primPath) &&
           _IsVisibleToCamera(prim.dataSource);
}

bool HdArnoldRenderPassSceneIndex::_RenderPassState::DoesPrune(SdfPath const &primPath) const
{
    return pruneEval && pruneEval->Match(primPath);
}

HdSceneIndexPrim HdArnoldRenderPassSceneIndex::GetPrim(const SdfPath &primPath) const
{
    // Pruning
    //
    ...
    if (_activeRenderPass.DoesOverrideVis(primPath, prim)) {
        static const HdContainerDataSourceHandle invisDs = HdRetainedContainerDataSource::New(
            ...
            HdVisibilitySchema::Builder().SetVisibility(HdRetainedTypedSampledDataSource<bool>::New(0)).Build());
    ...
    if (!primvarNames.empty()) {
        prim.dataSource = HdOverlayContainerDataSource::New(
            HdRetainedContainerDataSource::New(
                ...
                HdPrimvarsSchema::BuildRetained(primvarNames.size(), primvarNames.data(), primvarVals.data())),
            prim.dataSource);
    }
...
SdfPathVector HdArnoldRenderPassSceneIndex::GetChildPrimPaths(const SdfPath &primPath) const
{
...
inline static bool _EntryCouldAffectPass(const ENTRIES &entries, SdfPath const &activeRenderPassPath)
{
...
inline static bool _PruneEntries(
    std::optional<HdCollectionExpressionEvaluator> &pruneEval, const ENTRIES &entries, ENTRIES *postPruneEntries)
{
    if (!pruneEval) {
        // No pruning active.
        return false;
...
void HdArnoldRenderPassSceneIndex::_PrimsAdded(
...
void HdArnoldRenderPassSceneIndex::_PrimsRemoved(
...
void HdArnoldRenderPassSceneIndex::_PrimsDirtied(
...
HdArnoldRenderPassSceneIndex::~HdArnoldRenderPassSceneIndex() = default;

// Helper method to compile a collection evaluator.
static void _CompileCollection(
    HdCollectionsSchema &collections, TfToken const &collectionName, HdSceneIndexBaseRefPtr const &sceneIndex,
    SdfPathExpression *expr, std::optional<HdCollectionExpressionEvaluator> *eval)
{
...
void HdArnoldRenderPassSceneIndex::_UpdateActiveRenderPassState(
    ...
    HdSceneIndexObserver::RemovedPrimEntries *removedEntries)
{
    ...
    if (!state.renderPassPath.IsEmpty()) {
        ...
        if (HdCollectionsSchema collections = HdCollectionsSchema::GetFromParent(passPrim.dataSource)) {
            // Prepare evaluators for render pass collections.
            _CompileCollection(collections, _tokens->matte, inputSceneIndex, &state.matteExpr, &state.matteEval);
            _CompileCollection(
                collections, _tokens->renderVisibility, inputSceneIndex, &state.renderVisExpr, &state.renderVisEval);
            _CompileCollection(
                collections, _tokens->cameraVisibility, inputSceneIndex, &state.cameraVisExpr, &state.cameraVisEval);
            _CompileCollection(collections, _tokens->prune, inputSceneIndex, &state.pruneExpr, &state.pruneEval);
        }
    }

    // Short-circuit the analysis below based on which patterns changed.
    const bool visOrMatteExprDidChange = state.matteExpr != priorState.matteExpr ||
    ...
    for (const SdfPath &path : HdSceneIndexPrimView(_GetInputSceneIndex())) {
        if (priorState.DoesPrune(path)) {
            ...
            const bool primvarsDidChange =
                (priorState.DoesOverrideCameraVis(path, prim) != state.DoesOverrideCameraVis(path, prim)) ||
...
TF_REGISTRY_FUNCTION(TfType) { HdSceneIndexPluginRegistry ::Define<HdArnoldRenderPassSceneIndexPlugin>(); }
...
{
    return HdArnoldRenderPassSceneIndex::New(inputScene);
}
