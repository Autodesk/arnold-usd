...
SDR_REGISTER_PARSER_PLUGIN(SdrArnoldParserPlugin);
...
public:
    ArnoldShaderProperty(
        ...
        size_t arraySize, const SdrTokenMap& metadata, const SdrTokenMap& hints, const SdrOptionVec& options)
...
SdrArnoldParserPlugin::SdrArnoldParserPlugin() {}

SdrArnoldParserPlugin::~SdrArnoldParserPlugin() {}


void _ReadShaderAttribute(const UsdAttribute &attr, SdrShaderPropertyUniquePtrVec &properties, const std::string &folder)
{
...
SdrShaderNodeUniquePtr SdrArnoldParserPlugin::ParseShaderNode(const SdrShaderNodeDiscoveryResult& discoveryResult)
{
    auto shaderDefs = SdrArnoldGetShaderDefs();
    UsdPrim prim;
...
const SdrTokenVec& SdrArnoldParserPlugin::GetDiscoveryTypes() const
{
    static const SdrTokenVec ret = {_tokens->arnold};
    return ret;
}

const TfToken& SdrArnoldParserPlugin::GetSourceType() const { return _tokens->arnold; }
