...
//
// Modifications Copyright 2021 Autodesk, Inc.
//
...
const char UDIM_PATTERN[] = "<UDIM>";
constexpr int UDIM_START_TILE = 1001;
constexpr int UDIM_END_TILE = 1100;
constexpr std::string::size_type UDIM_TILE_NUMBER_LENGTH = 4;
...
bool _ResolveSymlinks(const std::string& srcPath, std::string* outPath)
{
    std::string error;
...
// Resolving symlinks can reduce the number of unique textures added into the
// texture registry since it may use the asset path as hash.
SdfAssetPath _ResolveAssetSymlinks(const SdfAssetPath& assetPath)
{
...
std::string _ResolvedPathForFirstTile(const std::pair<std::string, std::string>& splitPath, SdfLayerHandle const& layer)
{
    TRACE_FUNCTION();
...
// in the future.
std::pair<std::string, std::string> _SplitUdimPattern(const std::string& path)
{
...
SdfAssetPath _ResolveAssetAttribute(const SdfAssetPath& assetPath, const UsdAttribute& attr, const UsdTimeCode& time)
{
    TRACE_FUNCTION();
    ...
    return SdfAssetPath(
        ...
        firstTilePackage.empty() ? firstTilePath : ArJoinPackageRelativePath(firstTilePackage, firstTilePath));
}

TfToken _GetPrimvarNameAttributeValue(
    SdrShaderNodeConstPtr const& sdrNode, HdMaterialNode const& node, TfToken const& propName)
{
    VtValue vtName;
...
void _ExtractPrimvarsFromNode(
    HdMaterialNode const& node, HdMaterialNetwork* materialNetwork, TfTokenVector const& shaderSourceTypes)
{
...
TfToken _GetNodeId(
    UsdShadeConnectableAPI const& shadeNode, TfTokenVector const& shaderSourceTypes,
...
void _WalkGraph(
    UsdShadeConnectableAPI const& shadeNode, HdMaterialNetwork* materialNetwork, _PathSet* visitedNodes,
    TfTokenVector const& shaderSourceTypes, TfTokenVector const& renderContexts, UsdTimeCode time)
{
    ...
    for (UsdShadeInput input : shadeNodeInputs) {
        ...
        if (attrType == UsdShadeAttributeType::Output) {
            ...
            _WalkGraph(
                UsdShadeConnectableAPI(attr.GetPrim()), materialNetwork, visitedNodes, shaderSourceTypes,
                renderContexts, time);
            ...
            const VtValue value = UsdImagingArnoldResolveMaterialParamValue(attr, time);
            if (!value.IsEmpty()) {
...
bool _IsGraphTimeVarying(UsdShadeConnectableAPI const& shadeNode, _PathSet* visitedNodes)
{
    // Store the path of the node
    ...
    TfTokenVector const& renderContexts, HdMaterialNetworkMap* materialNetworkMap, UsdTimeCode time)
{
    ...
    _WalkGraph(UsdShadeConnectableAPI(usdTerminal), &network, &visitedNodes, shaderSourceTypes, renderContexts, time);
